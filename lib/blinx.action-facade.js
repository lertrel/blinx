...
import { isSNMFieldDef, extractSNMId } from './blinx.nested.js';
      ...
      function items() {
        ...
        function itemHandle(idOrIndex) {
          ...
          const resolveId = () => {
            ...
            if (index !== null) {
              try { return extractSNMId(collectionDef, getValueAt()); } catch { return null; }
            }
            try { return extractSNMId(collectionDef, idOrIndex); } catch { return null; }
          };
          const unlinkFromParentCollection = (targetId = null) => {
            if (!collectionDef || collectionDef?.type !== 'collection') return;
            const latest = getLatestArray();
            ...
            for (let i = 0; i < latest.length; i++) {
              ...
              if (targetId) {
                try { shouldRemove = extractSNMId(collectionDef, entry) === targetId; } catch { shouldRemove = false; }
            ...
            if (removed) setValue(next);
          };

          return {
            get: (k) => {
              const valueAt = getValueAt();
            ...
            delete: () => {
              ...
              try {
                ...
                touchedChildStores.add(childStore);
                unlinkFromParentCollection(resolvedId);
              } catch (e) {
                report({ code: 'SNM_CHILD_DELETE_FAILED', field: fieldKey, message: e?.message || String(e) });
              }
            },
