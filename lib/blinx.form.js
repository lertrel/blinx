...
function defaultBuiltinControlMeta(key) {
  switch (key) {
    case 'prevButton': return { label: 'Previous', css: 'blx-btn', variant: 'surface' };
    case 'nextButton': return { label: 'Next', css: 'blx-btn', variant: 'surface' };
    case 'saveButton': return { label: 'Save', css: 'blx-btn', variant: 'primary' };
    case 'resetButton': return { label: 'Reset', css: 'blx-btn', variant: 'surface' };
    case 'createButton': return { label: 'Create', css: 'blx-btn', variant: 'surface' };
    case 'deleteButton': return { label: 'Delete', css: 'blx-btn', variant: 'surface' };
...
function createDefaultToolbar(root) {
  ...
  if (meta.saveButton.variant) saveBtn.setAttribute('data-variant', meta.saveButton.variant);
  if (meta.resetButton.variant) resetBtn.setAttribute('data-variant', meta.resetButton.variant);
  if (meta.createButton.variant) createBtn.setAttribute('data-variant', meta.createButton.variant);
  if (meta.deleteButton.variant) deleteBtn.setAttribute('data-variant', meta.deleteButton.variant);
...
export function blinxForm({
  ...
  function supportsRendererForField({ ui, rendererName, fieldKey, fieldDef, mode }) {
    ...
    try {
      ...
      // Be conservative: if supportsField throws, treat as unsupported so we can fallback or fail.
      return false;
    }
  }

  function createSafeFieldWidget({ fieldKey, fieldDef, value: fieldValue, reason }) {
    const wrapper = document.createElement('div');
    wrapper.className = 'blx-field blx-field--unsupported';
    wrapper.setAttribute('data-blinx-part', 'field');
    wrapper.setAttribute('data-blinx-unsupported', 'true');
    if (reason) wrapper.setAttribute('data-blinx-unsupported-reason', String(reason));
    ...
    // Use default renderer formatting semantics for safety (e.g. mask secrets).
    input.textContent = String(getUI('default').formatCell(fieldValue, fieldDef) ?? '');
    ...
    return {
      el: wrapper,
      parts: { wrapper, label, input, error: errorEl },
      getValue: () => fieldValue,
      setError: () => {},
    };
  }
  ...
  // Theme boundary: consumers can override tokens at any scope by setting this attr on an ancestor.
  ...
  function syncEmptyStateControls() {
    ...
    const empty = total === 0;
    // When there are no records, allow Create, Reset but disable navigation + destructive actions.
    setDisabled(dom.prevBtn, empty);
    setDisabled(dom.nextBtn, empty);
    setDisabled(dom.saveBtn, empty);
    setDisabled(dom.deleteBtn, empty);
  }
  ...
  function buildSections() {
    ...
    view.sections.forEach(section => {
      ...
      sectionEl.className = 'blx-card';
      const title = document.createElement('div');
      title.className = 'blx-section-title';
      title.textContent = section.title;
      sectionEl.appendChild(title);

      const grid = document.createElement('div');
      grid.className = `blx-grid ${section.columns === 3 ? 'blx-grid-cols-3' : 'blx-grid-cols-2'}`;
      ...
      section.fields.forEach(f => {
        ...
        // Renderer capability check (optional).
        const requestedUI = getUI(rendererName);
        let uiToUse = requestedUI;
        let rendererToUse = rendererName;

        if (!supportsRendererForField({ ui: requestedUI, rendererName, fieldKey: key, fieldDef: def, mode: 'field' })) {
          const fallback = BlinxConfig.getUIFallbackRenderer();
          if (fallback && fallback !== rendererName) {
            const fallbackUI = getUI(fallback);
            if (supportsRendererForField({ ui: fallbackUI, rendererName: fallback, fieldKey: key, fieldDef: def, mode: 'field' })) {
              uiToUse = fallbackUI;
              rendererToUse = fallback;
            }
          }

          // If still unsupported, either fail (strict) or render safe fallback.
          if (uiToUse === requestedUI && rendererToUse === rendererName) {
            if (BlinxConfig.isUIRendererStrict()) {
              ...
              throw new Error(`blinxForm: renderer "${String(rendererName)}" does not support field "${String(key)}" (type "${t}") for model "${m}".`);
            }
            uiToUse = null;
          }
        }

        const widget = uiToUse
          ? uiToUse.createField({
          fieldKey: key,
          def,
          value,
          ...
          : createSafeFieldWidget({
            fieldKey: key,
            fieldDef: def,
            value,
            reason: `renderer "${String(rendererName)}" unsupported`,
          });
