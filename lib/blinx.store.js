...
import {
  isSNMFieldDef,
  normalizeValueForDiff,
  normalizePatchForPersist,
  normalizeRecordForPersist,
  normalizeValueForPersist,
} from './blinx.nested.js';
...
function createLegacyArrayStore(initialArray, dataModel) {
  ...
  let original = clone((initialArray || []).map(r => normalizeRecordForPersist(dataModel, stripComputedFields(dataModel, r))));
  let current = clone((initialArray || []).map(r => normalizeRecordForPersist(dataModel, stripComputedFields(dataModel, r))));
  ...
  function setField(idx, field, value) {
    ...
    const next = (def && isSNMFieldDef(def)) ? normalizeValueForPersist(def, value) : value;
    current[idx][field] = next;
    invalidateComputedForField(model, current[idx], field);
    notify([idx, field], next);
  }

  function addRecord(record, atIndex = current.length) {
    const sanitized = normalizeRecordForPersist(model, stripComputedFields(model, record || {}));
    ...
    for (let i = 0; i < max; i++) {
      ...
      if (rec && orig) {
        for (const k of Object.keys(rec)) {
          const a = normalizeValueForDiff(model, k, rec[k]);
          const b = normalizeValueForDiff(model, k, orig[k]);
...
function createRemoteViewStore({ model, dataSource, viewName, viewConfig }) {
  ...
  async function cacheWarmAll({ pageSize = 200, maxPages = 2000, criteriaOverride = null } = {}) {
    ...
    for (; pages < maxPages; pages++) {
      ...
      const fetched = (res?.entities?.[entityType] || []).map(r => normalizeRecordForPersist(model, stripComputedFields(model, clone(r))));
      cacheIngest(fetched);
  ...
  function setField(idx, field, value) {
    ...
    if (id !== undefined && id !== null) {
      const patch = normalizePatchForPersist(model, { [field]: value });
      // Avoid sending no-op SNM relationship updates (e.g., label-only changes).
      if (def && isSNMFieldDef(def)) {
        const prev = normalizeValueForDiff(model, field, original?.[idx]?.[field]);
        const now = normalizeValueForDiff(model, field, next);
        if (JSON.stringify(prev) !== JSON.stringify(now)) {
          enqueueUpdate(id, baseVersion, patch);
        }
      } else {
        enqueueUpdate(id, baseVersion, patch);
      }
  ...
  function addRecord(record, atIndex = current.length) {
    const rec = normalizeRecordForPersist(model, stripComputedFields(model, clone(record || {})));
    ...
    enqueue({
      ...
      data: clone(normalizeRecordForPersist(model, rec)),
    });
  ...
  function updateById(id, patch, { baseVersion = null } = {}) {
    ...
    if (!p) return false;
    const persistPatch = normalizePatchForPersist(model, p);
    const idx = current.findIndex(r => String(r?.[keyField]) === sid);
    ...
    const applyPatchToRecord = (rec) => {
      ...
      for (const [k, v] of Object.entries(p)) {
        ...
        rec[k] = (def && isSNMFieldDef(def)) ? normalizeValueForPersist(def, v) : v;
    ...
    if (idx >= 0 && current[idx]) {
      ...
      const effectivePatch = { ...persistPatch };
      if (original[idx]) {
        for (const [k] of Object.entries(persistPatch)) {
          const def = model?.fields?.[k];
          if (def && isSNMFieldDef(def)) {
            const prev = normalizeValueForDiff(model, k, original[idx]?.[k]);
            const now = normalizeValueForDiff(model, k, current[idx]?.[k]);
            if (JSON.stringify(prev) === JSON.stringify(now)) {
              delete effectivePatch[k];
            }
          }
        }
      }
      ...
      if (Object.keys(effectivePatch).length > 0) {
        enqueueUpdate(sid, bv, effectivePatch);
      }
    ...
    enqueueUpdate(sid, bv, persistPatch);
    // Update cache if present.
    const existing = cacheConfig && cacheState ? cacheState.entities.get(sid) : null;
    if (existing && typeof existing === 'object') {
      const merged = { ...existing, ...persistPatch, [keyField]: sid };
      cacheIngest([merged]);
    }
    notify([EventTypes.reset, 0], current[0] || null);
    return true;
  ...
  function update(index, record) {
    if (index < 0 || index >= current.length) return null;
    current[index] = normalizeRecordForPersist(model, stripComputedFields(model, clone(record)));
  ...
  async function loadFirst(nextCriteria = null) {
    ...
    const records = (res?.entities?.[entityType] || []).map(r => normalizeRecordForPersist(model, stripComputedFields(model, clone(r))));
    cacheIngest(records);
    current = records;
  ...
  function localView(localViewKey) {
    ...
    function sourceSnapshot() {
      ...
      try {
        return (storeApi?.toJSON ? storeApi.toJSON() : []).map(r => normalizeRecordForPersist(model, stripComputedFields(model, clone(r))));
      } catch {
        return [];
      }
    }
    ...
    async function materialize({ resetOffset = false } = {}) {
      ...
      const records = sourceSnapshot().map(r => normalizeRecordForPersist(model, stripComputedFields(model, clone(r))));
  ...
  async function save() {
    ...
    if (canonical && Array.isArray(canonical)) {
      ...
      const byId = new Map(canonical.map(r => [String(r?.[keyField]), normalizeRecordForPersist(model, stripComputedFields(model, clone(r)))]));
