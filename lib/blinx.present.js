...
export function bestEffortFindPart(root, part) {
  ...
  // Preferred: stable, renderer-agnostic part hooks.
  // These are designed to work across multiple co-existing renderers without relying on class names.
  if (part === 'input') {
    return root.querySelector('[data-blinx-part="input"]') || root.querySelector('input, textarea, select');
  }
  if (part === 'label') {
    return root.querySelector('[data-blinx-part="label"]') || root.querySelector('label');
  }
  if (part === 'error') {
    return (
      root.querySelector('[data-blinx-part="error"]') ||
  ...
  if (part === 'help') {
    return (
      root.querySelector('[data-blinx-part="help"]') ||
      // Backwards-compatible fallbacks:
      root.querySelector('.help, .hint, [data-help]')
    );
  }
  return null;
}
